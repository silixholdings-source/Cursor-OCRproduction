#!/usr/bin/env python3
"""
Simple Service Test - Tests Frontend and Backend
"""
    ...
    try:
        with urllib.request.urlopen(url, timeout=5) as response:
            if response.status == 200:
                print(f"✅ {name}: Working (HTTP {response.status})")
                return True
            else:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            if response.status == 200:
                ...
                if result.get("status") == "success":
                    print("✅ Backend OCR Demo: Working")
                    return True
                else:
                    print("❌ Backend OCR Demo: Invalid response")
                    return False
            else:
                print(f"❌ Backend OCR Demo: Failed (HTTP {response.status})")
                return False
    except Exception as e:
        print(f"❌ Backend OCR Demo: Connection failed - {e}")
        return False
