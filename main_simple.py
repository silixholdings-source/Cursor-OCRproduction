"""
Simple OCR Automation Service - Works without Tesseract for testing
"""
import logging
import tempfile
import os
...
class Settings(BaseSettings):
    ...
    # OCR Configuration
    OCR_PROVIDER: str = "mock"  # mock, tesseract, azure
...
class OCRProcessor:
    ...
    async def extract_text_from_image(self, image_path: str, language: str = "eng") -> Dict[str, Any]:
        ...
        try:
            ...
            # Mock OCR result
            mock_text = f"Mock OCR result for {Path(image_path).name}\nThis is simulated extracted text from the image."
            
            return {
                "text": mock_text,
                "confidence": 0.85,
                "filtered_text": mock_text,
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"OCR processor initialized with {settings.OCR_PROVIDER}")
